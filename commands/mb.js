...
function help_info(prefix) {
  ...
  help["help"] += `    Usage: \`${prefix}${jsCommand} [name][add name type][remove name][sheet]\`\n`;
...
function insert(client, db, name, type) {
  return new Promise( (resolve, reject) => {
    ...
    db.update({"name": name }, entry, { upsert: true }, function(err, newDoc) {
        if(!err) {
          resolve(`<@${client}> Added personality [${type.toUpperCase()}] for: ${name}` );
        } else {
...
function execute(rawArgs) {
  return new Promise( (resolve, reject) => {
    var key = rawArgs.key;
    var db = new Datastore({ filename: `.data/mbDatafile_${key}`, autoload: true });
